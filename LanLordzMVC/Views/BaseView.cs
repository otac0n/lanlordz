...
namespace LanLordz.Views
{
    public abstract class BaseView<TModel> : SparkView<TModel> where TModel : ControllerResponse
    {
        ...
        protected string UserLink(User user)
        {
            return this.UserLink(user.UserID, user.Username, user.Username);
        }

        protected string UserLink(UserInformation user)
        {
            return this.UserLink(user.UserID, user.Username, user.Username);
        }

        protected string UserLink(long userId, string username)
        {
            return this.UserLink(userId, username, username);
        }

        protected string UserLink(long userId, string username, string displayName)
        {
            var routeValues = new
            {
                id = userId,
