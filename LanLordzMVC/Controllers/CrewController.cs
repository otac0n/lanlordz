    ...
    public class CrewController : LanLordzBaseController
    {
        ...
        public ActionResult DoorPrizes(long? id, FormCollection values)
        {
            ...
            else if (action == "DRW")
            {
                ...
                var minWins = (from r in this.Db.Registrations
                               where r.EventID == id.Value
                               select this.Db.Prizes.Where(p => p.EventId == r.EventID && p.WinnerUserId == r.UserID).Count()).Min();

                var users = (from r in this.Db.Registrations
                             where r.EventID == id.Value
                             where r.IsCheckedIn
                             where this.Db.Prizes.Where(p => p.WinnerUserId.HasValue && p.EventId == r.EventID && p.WinnerUserId == r.UserID).Count() == minWins
